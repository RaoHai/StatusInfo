...
--print("Geting")
tmaintalent=0
tnumTabs = GetNumTalentTabs();
Stance=GetShapeshiftForm();
tplayerClass = string.upper(select(2, UnitClass('player')));
--print(tplayerClass,Stance,tnumTabs)
tmaintalent=GetPrimaryTalentTree()
if tplayerClass== "DRUID" then
	...
	elseif tmaintalent==2 then
		if Stance== 1 then PlayAs=4 
			elseif Stance== 3 then PlayAs=1 end
...
if PlayAs==1 then
	...
	statu2:SetText("|cff00ffff"..Monitor(L["Mastery"],GetCombatRatingBonus(26)))
...
elseif PlayAs==2 then
	...
	statu4:SetText(format("|cffcc9966%.2f%%",GetCombatRatingBonus(8)))
	statu5:SetText("|cff00ffff"..Monitor(L["Mastery"],GetCombatRatingBonus(26)))
...
local effective = base + posBuff + negBuff;
	...
	statu2:SetText(format("|cff00ff00 %.2f%%",GetCombatRatingBonus(26)))
	...
	statu5:SetText(format("|cffcc9966%.2f%%",GetCombatRatingBonus(7)))
	statuMainDes:SetText(L["AttactPower"])
	statu2Des:SetText(L["Mastery"])
	statu3Des:SetText(L["Crit"])
	statu4Des:SetText(L["AttackSpeed"])
